...
async function fetchWikipediaSummary(topic) {
    // NEW: Augment the topic to force a medical search result
    const augmentedTopic = `${topic} medical condition OR human disease`;

    // 1. Search for the topic to get a page title
    const searchParams = new URLSearchParams({
        action: 'query',
        list: 'search',
        srsearch: augmentedTopic, // Use the augmented topic here
        format: 'json',
    ...
    try {
        ...
        return { error: "Search failed due to network issue." };
    }
